...
const CanalReport: React.FC<CanalReportProps> = ({
    ...
    // Genera el reporte en una nueva pestaña vía Blob URL y dispara window.print() ahí.
    // Tres razones para NO usar window.open() ni useReactToPrint:
    //   1. useReactToPrint → window.print() en el modal bloquea el thread JS (freeze total).
    //   2. window.open() puede ser bloqueado por el popup blocker del navegador.
    //   3. Template literal con `${bodyHtml}` interpola texto del DOM como JS: si el
    //      reporte contiene "${algo}" (constantes, nombres) se evalúa y lanza un error
    //      que deja ambos botones sin respuesta.
    // Solución: Blob + <a>.click() — equivalente a "Guardar como", nunca bloqueado.
    const handlePrint = () => {
        const bodyEl = ref.current;
        if (!bodyEl) return;

        // Valores CSS concretos (no template literals aplicados al DOM dinámico)
        const semC  = semColor(iec.semaforo);
        const pef   = ((iec.p_eficiencia / 30) * 100).toFixed(1) + '%';
        const pcoh  = ((iec.p_coherencia / 25) * 100).toFixed(1) + '%';
        const pfug  = ((iec.p_fugas      / 25) * 100).toFixed(1) + '%';
        const pcrit = ((iec.p_criticos   / 20) * 100).toFixed(1) + '%';

        // Reemplazar ruta relativa del logo por URL absoluta en la copia del DOM
        const bodyHtml = bodyEl.innerHTML
            .split('/logos/logo-srl.png')
            .join(window.location.origin + '/logos/logo-srl.png');

        // CSS embebido — construido con concatenación, sin template literal en el body HTML
        const css = '@page{size:A4;margin:14mm 12mm}'
        ...
        // Blob + anchor click: nunca bloqueado por popup blocker (equivalente a descarga)
        ...
        // Revocar el URL después de 30s para liberar memoria
