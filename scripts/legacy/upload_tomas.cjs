...
async function upload() {
    ...
    for (const line of lines) {
        ...
        const capacidad = capacidadStr === '0' ? 0.0 : (parseFloat(capacidadStr) / 1000);

        if (tipo === 'directa' || tipo === 'represa' || tipo === 'descarga') tipo = 'toma';
        if (tipo === 'bombeo') tipo = 'carcamo';
        ...
        else { seccion_id = 'SEC-004'; seccion_texto = '23'; }
    ...
    const { data: result, error } = await supabase.from('puntos_entrega').insert(inserts);
    if (error) {
